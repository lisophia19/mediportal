...

{
  ...
  echo 'apt-get install -y docker.io docker-compose-plugin git curl debian-keyring debian-archive-keyring apt-transport-https gnupg'
  ...
  echo '# seed_mediportal.py expects to sit next to a backend/ dir and the'
  echo '# xlsx (repo-root layout) -- inside the container /app IS backend/'
  echo '# itself, so recreate that layout via a symlink before running it.'
  echo 'docker compose exec -T backend mkdir -p /opt/seed_run'
  ...
  echo 'docker compose cp "Mediportal Information FINAL.xlsx" "backend:/opt/seed_run/Mediportal Information FINAL.xlsx"'
