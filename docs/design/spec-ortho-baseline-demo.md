...
`matched` / `needs_triage` / `no_match`, and `slots_available` / `no_slots`.
...
```json
{ "status": "needs_triage",
  "triage_question": "Is this from a recent injury or fall, or is it pain that's built up over time?",
  "term_a_id": 4, "term_a_label": "a possible broken wrist",
  "term_b_id": 7, "term_b_label": "ongoing wrist pain" }
```
...
`needs_triage`'s `triage_question` is **generated live** for the specific ambiguous
pair (`_generate_triage_question`, same Anthropic API call as classification) — not a
fixed question or a hardcoded list of ambiguous pairs. `POST /routing/resolve-triage`
(`triage_answer`, `triage_question`, `term_a_id`, `term_b_id`) classifies the answer
and either resolves to a term or — on a genuinely unclear answer — returns a *new*
`still_unclear` + follow-up `triage_question`, informed by what was already asked so it
doesn't repeat itself. The Vogent flow caps this at 3 rounds before falling back to an
honest callback offer; `resolve-triage` itself never forces a guess.

`confirm_prompt`, `triage_question`, and `spoken_response` are **generated by the
...
| 4a | Triage | On `needs_triage`, speaks the generated `triage_question`, captures answer, calls `resolve-triage`. A `still_unclear` answer generates a new follow-up question and repeats — **max 3 rounds**, then falls through to an honest callback offer rather than a forced guess. | `POST /routing/resolve-triage` |
...
| Complaint is ambiguous between two terms | `needs_triage` with a live LLM-generated discriminating question for that specific pair (no hardcoded pairs or fixed question text); up to 3 rounds, then an honest callback offer, never a forced guess. |
